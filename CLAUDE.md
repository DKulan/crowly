...
Crowly is a native iOS **inbox/reader** for recurring AI-agent/automation outputs — AI news summaries, weather, local community updates, scheduled briefings, reminders. Stage: **reader-only pivot** (2026-06-29), M1 build complete and live end-to-end — the iOS app is built and verified on-device (unpaired = demo fixtures; paired = live companion pull, including a live-data home-screen widget as of Phase 1, 2026-07-02), and the design docs were rewritten away from the earlier "job-bound response loop" framing. What remains before M2 is the two-week behavioral validation, not more building. The repo/dir is `agent-output-inbox`; the product ships as **Crowly**.
