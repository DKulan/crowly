...
- **Crow art + motion (M2, branch `feat/onboarding-animations`).** The onboarding carousel is a **deliberately rich** surface — the one place in the app where motion carries personality (the inbox, detail, and widget stay calm; see the "rich onboarding, calm everywhere else" rule in `docs/design-system.md` § 4.4). All of it is native SwiftUI (iOS 26) — **no Lottie, no third-party animation dependency** — and all of it honors Reduce Motion:
  - **Layered crow hero.** `App/Views/Onboarding/CrowAnimationView.swift` composes the crow from three transparent PNG layers on a shared canvas — `crow-body` / `crow-lines` / `crow-wing`, extracted from the app icon (`App/Assets.xcassets`) — driven by a single `KeyframeAnimator` running independent tracks: the whole crow bobs / tilts / breathes, the wing flaps around the shoulder pivot, and the speed-lines drift + pulse. Each of the four `CrowKind` beats (welcome / companion / hermes / pair) varies the *motion* (a `CrowMotionProfile`), not the art. If the layer assets are missing it degrades gracefully to the flat single `crow` asset (the earlier code-driven bob), then to an SF Symbol.
  - **Staged reveals.** When a page becomes active its elements arrive in a gentle stagger — crow hero → headline → divider → body — and the reveal replays on swipe-back so the deck always feels alive (`RevealModifier` in `OnboardingView.swift`).
  - **Morphing page indicator.** The active orange pill *slides* between dots via `matchedGeometryEffect` instead of popping (`OnboardingPageIndicator.swift`); each slot reserves the pill's footprint so the row never reflows.
  - **Breathing field.** The flat cream backdrop is now a very subtle animated `MeshGradient` in the brand palette (cream / creamDeep / paper + one whisper of warm glow), drifting on a slow loop (`OnboardingBackground.swift`). Warmth, not a light show — it never brightens or flips.
  - **Hero hand-off.** Finishing onboarding lifts + scales + fades the whole composition as the inbox is revealed underneath (`OnboardingView.finish()` + `ContentView`), reading as one continuous "lifting off into the app" motion rather than a flat opacity cross-fade. This is a **scoped, deliberate exception** to the "avoid custom navigation/zoom transitions in M1" note below — it applies *only* to the onboarding → inbox hand-off and is **not** a license for custom transitions inside the reader (inbox/detail stay calm).
  - **Pairing-success beat.** A one-off `.symbolEffect(.bounce)` on the checkmark in `PairCompanionView` as the pair lands.
  - **Reduce Motion** holds all of the above still: the crow at rest, the mesh static, the pill's move instant, staged reveals reduced to a plain cross-fade, and the hero hand-off skipped (the parent's cross-fade still runs). See `docs/design-system.md` § 4.4.
  - `CrowKind` remains as an enum (one case per screen); the layers are shared art and each kind varies only the motion profile.
- **Visual identity.** The whole carousel sits on the fixed brand palette — a warm cream field (the gently breathing `MeshGradient` above, not a flat fill), ink serif headline (`Font.crowlyDisplay*`), a `CrowlyDivider` (short orange rule + center dot) under each headline, orange brand pill CTA, and the morphing orange page indicator on warm hairline dots. The app is light-locked (`.preferredColorScheme(.light)` at the scene root) so onboarding reads the same in light and dark system appearance. Full palette + type rules live in `docs/design-system.md`.
...
**Avoid in M1:** custom navigation/zoom transitions **inside the reader** (inbox ↔ detail use system pushes; the sole sanctioned exception is the onboarding → inbox hero hand-off in § Onboarding, which is scoped to first-run only); **Lock Screen widgets** (defer — they add platform surface without changing the read flow); background URL session for state writes (synchronous POST + retry-on-foreground is enough); `Button(intent:)` widgets (no action surface in a reader).
