...
- **First-run onboarding** — a 4-screen carousel (`App/Views/Onboarding/`) shown once on first launch (gated by `@AppStorage("hasOnboarded")`), then handing off to pairing or dismissing into demo mode (`docs/ux.md` § Onboarding). Onboarding art is Lottie-backed but currently a SwiftUI placeholder (real crow `.lottie` assets not sourced yet).
...
- **Third-party dependencies** — `lottie-ios` (SwiftPM, ≥4.5.0; **app target only**; backs the onboarding crow animations, currently placeholder art) and Apple's **VisionKit** framework (QR pairing scanner; needs `NSCameraUsageDescription`, declared in `project.yml`). The widget and test targets pull in neither.
