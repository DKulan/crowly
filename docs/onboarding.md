...
> 2. **Install the `setup-crowly` skill from the Hermes Skills Hub and ask Hermes to run it** — `hermes skills install DKulan/crowly/skills/setup-crowly --force`. It fetches the companion source itself (a pinned clone — Step 0 below), detects the host, stands up the companion, gives it a Tailscale-Funnel HTTPS address, mints a pairing token + renders a pairing QR *locally on the host*, and installs the `emit-crowly-digest` skill + a starter cron so the inbox isn't empty on first open. (No need to pre-clone the monorepo — the skill pulls the companion source in Step 0.)
...
| 4. Wire the emitter | ✅ helper against real companion + `setup-crowly` skill built (`emitter/hermes-skill/setup-crowly/`), distributed via the Skills Hub (`hermes skills install DKulan/crowly/skills/setup-crowly --force`, `docs/publishing-skills.md`) | Writing the cron's digest content (per-user) |
