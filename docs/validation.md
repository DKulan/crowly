...
2. Make 2–3 Hermes jobs emit it via the emitter kit — start with **Harmony digest** (clean, low-urgency, weekly), plus **morning briefing** (daily, longer-form) and one **vendor/research watcher** (irregular, occasionally high-urgency). The mix exercises chronological scan, the urgency glyph, and widget surfacing.
3. Stand up the **companion + app** (M1): inbox list + detail + archive + the **home-screen widget** (pull/timeline-refresh).
...
- **Add the app and the medium widget to the home screen on day one** — the widget *is* the cue (it's the only one the MVP has).
...
- If after two weeks Daniel does **not** reach for the app or look at the widget unprompted, **stop at M1** — do not build the public layer (demo polish, stranger TLS, privacy) or widen to generic webhooks. Single-user pull is the real killer, and M1 is the cheapest place to learn it.
...
Because M1 gates M2, a null result costs M1, not the whole public build.
